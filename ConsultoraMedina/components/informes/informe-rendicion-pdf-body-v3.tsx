...
const s = StyleSheet.create({
  ...
  subUnidadLine: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  subUnidadBold: { fontSize: 9, fontWeight: "bold" },
  subUnidadNum: { fontSize: 9, fontWeight: "bold", textAlign: "right" },
  ...
  closureLine: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 6 },
    ...
    <>
        ...
        {payload.unidades.length === 0 ? (
          ...
          payload.unidades.map((u) => (
            <View key={u.pago_id} style={s.unidadWrap} wrap={false}>
              <Text style={s.unidadTitle}>Propiedad: {u.etiqueta}</Text>
              {u.lineas.map((row, i) => (
              ...
              <View style={s.subUnidad}>
                <View style={s.subUnidadLine}>
                  <Text style={s.subUnidadBold}>Subtotal cobrado al inquilino</Text>
                  <Text style={s.subUnidadNum}>{precio(u.subtotal_cobrado_inquilino)}</Text>
                </View>
              </View>
            </View>
          ))
      ...
      <View style={s.inmobBlock}>
        ...
        <View style={s.tableFootLight}>
          <View style={s.subUnidadLine}>
            <Text style={s.subUnidadBold}>Suma conceptos inmobiliaria</Text>
            <Text style={s.subUnidadNum}>{precio(payload.total_suma_inmobiliaria_conceptos)}</Text>
          </View>
        </View>
      </View>
      ...
      <View style={s.closureBox}>
        ...
        <View style={s.closureLine}>
          ...
          <Text style={s.closureNum}>{precio(payload.subtotal_a_rendir_propietario)}</Text>
        </View>
        <View style={s.closureLine}>
          <Text style={s.closureBold}>Comisión Inmobiliaria ({payload.comision_porcentaje}%)</Text>
          <Text style={s.closureNum}>- {precio(payload.comision_monto)}</Text>
        </View>
